...
<jsp:include page="/WEB-INF/views/include/header_filo.jsp" />
            ...
            <div class="inputField">
                <form action="/filo/member/loginPro.fl" method="post">                
                    ...
                    <div class="inputOuter inputOuter2">
                    	<p class="tit">아이디</p>
                        <input type="text" id="idInput" name="id" placeholder="숫자 혹은 영어로 이루어진 6~12자리" maxlength="12"/>
                    ...
                    <div class="inputOuter inputOuter2">
                        ...
                        <input type="password" id="pwChInput" name="pwCh"/>
                    ...
                    <div class="inputOuter inputOuter2">
                    	<p class="tit">생년월일</p>
                        <input type="text" id="birthInput" name="birth" placeholder="8자리 생년월일(ex.19920428)"/>
	            ...
	            <div class="signArticle">
	                ...
	                <form action="/filo/joinPro.fl" id="signupForm" onsubmit="return subCheck()" method="post">
	                    <div class="formBox">
	                        ...
	                        <div class="inputArea">
	                            <label for="idInput" class="inputPlace  screenIn">아이디(숫자 혹은 영어로 이루어진 6~12자리)</label>
	                    ...
	                    <div class="formBox">
	                        ...
	                        <div class="inputArea">
	                            <label for="pwInput" class="inputPlace  screenIn">비밀번호(숫자 혹은 영어로 이루어진 6~12자리)</label>
	                    ...
	                    <div class="formBox">
	                        ...
	                        <div class="inputArea">
	                            <label for="nickInput" class="inputPlace  screenIn">닉네임이(숫자 혹은 영어로 이루어진 2~12자리)</label>
	                    ...
	                    <div class="formBox">
	                        ...
	                        <div class="inputArea">
	                            <label for="nameInput" class="inputPlace  screenIn">이름</label>
    ...
    $('.inputOuter > select').focusin(function(){
        $(this).addClass('on');
    });
    $('.inputOuter > select').focusout(function(){
        $(this).removeClass('on');
    });
	...
	let idPattern = /^[a-zA-Z0-9]{6,12}$/;	//아이디	a~z,A-Z,0~9로 시작하는 6~12자리 아이디를 만들 수 있다.
	let pwPattern = /^[a-zA-Z0-9]{6,12}$/;	//비번	동일
	let nickPattern = /^[a-zA-Z가-힣0-9]{2,12}$/;	//닉네임	a~z,0~9로 시작하는 2~12자리 아이디를 만들 수 있다.
	let emailPattern = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])+.[a-zA-Z]{2,3}$/;	//email	
	...
	function subCheck(){
		...
		if(!regExCheck(idPattern,idInput)){
			$('.idError1').text('숫자 혹은 영어로 이루어진 6~12자리 아이디를 만들어주세요.');
			return false;	
		}
		//아이디 중복 체크
		...
		if(!regExCheck(pwPattern,pwInput)){
			$('.pwError').text('숫자 혹은 영어로 이루어진 6~12자리 비밀번호를 만들어주세요.');
			return false;
		}
		//비밀번호 중복 체크 
		...
		if(!regExCheck(nickPattern,nickInput)){
			$('.nicknameError').text('2~12자리 닉네임을 만들어주세요.');
			return false;
		}
		//email
		checkExist(emailInput,"email을");
		if(!regExCheck(emailPattern,emailInput)){
			$('.emailError').text('형식에 맞게 e-mail을 작성해주세요');
			return false;
		}
		//이메일 중복 체크 
		...
		if(!regExCheck(namePattern,nameInput)){
			$('.nameError').text('이름이 형식에 맞지 않습니다.');
			return false;
		}
		//생년월일
		checkExist(birthInput,"생일을");
		if(!regExCheck(birthPattern,birthInput)){
			$('.birthError').text('생년월일을 올바르게 입력해주세요 ex)19920428');
			return false;
		}
		//성별
	}
	...
	$('#idInput').change(function(){
		if(regExCheck(idPattern,idInput)){
			...
			$('.idError1').text('숫자 혹은 영어로 이루어진 6~10자리 아이디를 만들어주세요.');
	...
	$('#pwInput').change(function(){
		if(regExCheck(pwPattern,pwInput)){
			...
			$('.pwError').text('숫자 혹은 영어로 이루어진 6~10자리 비밀번호를 입력해주세요.');
	...
	$('#pwChInput').change(function(){
		console.log('change');
		if(pwInput.value == pwChInput.value){
			console.log('!=');
			$('.pwError1').text('');
			$('#pwChInput').removeClass('errorBor');
		}else{
			$('.pwError1').text('비밀번호가 서로 일치 하지 않습니다.');
			$('#pwChInput').addClass('errorBor');
		}
	});
	//닉네임 실시간 확인
	...
	$('#emailInput').change(function(){
		if(regExCheck(emailPattern,emailInput)){
			...
			$('.emailError1').text('형식에 맞는 e-mail주소를 입력해주세요');
	...
	$('#nameInput').change(function(){
		if(regExCheck(namePattern,nameInput)){
			...
			$('.nameError').text('올바른 형식의 이름을 입력해주세요');
