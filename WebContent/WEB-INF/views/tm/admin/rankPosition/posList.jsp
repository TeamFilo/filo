...
<%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
	...
	<script>
				...
				$.ajax({
					...
					success: function(res){
						...
						for(var i=0 ; i<posSel.sPosList.length ; i++ ){
							...
							$('#list').append("<td><input type='button' value='수정' onclick=\"window.location='/filo/admin/modifyFormPos.fl?posNo="+posSel.sPosList[i].posNo+"'\"/></td>");
						}
		...
		//삭제 함수
   		function dee(index){
   			...
   			if(check){
   				location.href="/filo/admin/deletePos.fl?posNo="+index;
   			}else{
   				location.href="/filo/admin/posList.fl";
   			}
   		}
	   	//유효성 검사
	   		function check(){
	...
	<div class="wrapAll admin">
		...
		<br/><br/>
		<!-- 포지션 추가  -->
		<form action="/filo/admin/addSPosPro.fl" name="addPosForm" onsubmit="return check()" method="post" enctype="multipart/form-data">
			<table class="tm">
	    		<tr>
					<th>포지션 대분류</th>
					<th>이모티콘</th>
		...
		<c:if test="${count>0}">
			...
			<c:forEach var="spo" items="${sPosList}">
	    		<tr>
	    			...
	    			<td>${spo.posExpl}</td>
	    			<td >
	    				<input type="button" value="수정" onclick="window.location='/filo/admin/modifyFormPos.fl?posNo=${spo.posNo}'"/>
		...
		<div align="center">
			...
			<c:if test="${startPage>pageBlock}">
				<a href="/filo/admin/posList.fl?pageNum=${startPage-pageBlock}" class="pageNums"> &lt;</a>
			</c:if>
			
			<!-- 페이지 리스트 -->
			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
				<a href="/filo/admin/posList.fl?pageNum=${i}" class="pageNums"> &nbsp; ${i} &nbsp;</a>
			</c:forEach>
