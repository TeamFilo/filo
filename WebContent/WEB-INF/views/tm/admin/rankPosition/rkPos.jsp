...
<%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
	...
	<!-- //adminMenu end -->
   	<script>
   		function de(index){
   			...
   			if(check){
   				location.href="/filo/admin/deleteRk.fl?rkNo="+index;
   			}else{
   				location.href="/filo/admin/rkPos.fl";
   			}
   		}
	...
	<div class="wrapAll admin">
		...
		<form action="/filo/admin/addRkPro.fl" name="addRankForm" onsubmit="return check()" method="post">
			<table class="tm">
	    		<tr>
					<th>등급이름</th>
					<th>요구횟수</th>
		...
		<c:if test="${fn:length(rkList)!=0}">
   		...
   		<c:forEach var="rk" items="${rkList}">
	    		<tr class="tm">
	    			...
	    			<td>${rk.rkAddExpl}</td>
	    			<td>
	    				<input type="button" value="수정" onclick="window.location='/filo/admin/modifyFormRk.fl?rkNo=${rk.rkNo}'"/>
