...
<div class="myPageWrap" id="transcroller-body">
	...
	<c:if test="${empty travelAll}">
		...
		<div class="btnWrap fl">
			<p onclick="window.location='/filo/travel/makingList.fl'" class="btn btnY">여행떠나기</p>
		</div>
	</c:if>
	<c:if test="${!empty travelAll}">
		...
		<div class="infoList">
			<c:forEach var="travelAll" items="${travelAll}" varStatus="status">
		             <ul>
		                 <li>
		                     <div>
		                         <h2>${travelAll.subject} <a href="/filo/my/myHistoryCont.fl?gNo=${travelAll.gNo}&status=${travelAll.status}" class="posA">더 알아보기 ></a></h2>
		                         <dl>
		                             <dt>날짜</dt>
				...
				<%-- 	No.${status.count} </br>
					<a href="/filo/my/myHistoryCont.fl?gNo=${travelAll.gNo}&status=${travelAll.status}"><h3>${travelAll.subject}</h3></a>
	...
	<c:if test="${!empty lsharedList}">
		<form action="/filo/shared/sharedLikedCancel.fl" name="cancelLiked" method="post" onsubmit="return cancelCheck()" >
