	...
	<c:if test="${count > 0 }">	
		<div id="sList">
			<div id="articleTable">
				<ul class="makingList">
					<c:forEach var="article" items="${sharedList}" varStatus="status">
						<li>
							<a href="completedCont.fl?gNo=${article.gNo}&pageNum=${pageNum}"></a>
			...
			<c:if test="${startPage > pageBlock}">
				<a href="sharedList.fl?pageNum=${startPage-pageBlock}&start=${start}&end=${end}"> &lt; </a>
			</c:if>
			<c:if test="${start != null && end != null}">
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="sharedList.fl?pageNum=${i}&start=${start}&end=${end}">&nbsp; ${i} &nbsp;</a>
				</c:forEach>
			</c:if>
			<c:if test="${start == null && end == null}">
				<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
					<a href="sharedList.fl?pageNum=${i}">&nbsp; ${i} &nbsp;</a>
				</c:forEach>
			</c:if>
			<!-- 뒤로 가는 기호 -->
			<c:if test="${endPage < pageCount}">
