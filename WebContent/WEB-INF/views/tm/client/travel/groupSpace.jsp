...
<jsp:include page="/WEB-INF/views/include/header.jsp" />
   ...
   <div id="transcroller-body" class="wrapAll client">
      ...
      <c:if test="${idStatus!=1}">
         <script>
            ...
            location.href="/filo/travel/makingList.fl";
         </script>
      </c:if>
      ...
      <div class="gsWrap">
         ...
         <ul class="gsMenu">
            <li class="on"><a href="groupSpace.fl?gNo=${gNo}">그룹홈</a></li>
            <li><a href="gallery.fl?gNo=${gNo}">갤러리</a></li>
         </ul>
            ...
            <ul class="list">
               <li>
                  ...
                  <div class="inner">
                     ...
                     <p class="txt">
                        ${mem.id}님
                        <c:if test="${mem.id!=id}">
                           <input type="button" onclick="window.location='/filo/msg/messageWrite.fl?receiver=${mem.id}&sender=${id}'" value="쪽지쓰기"/>
                        </c:if>
                     </p>
                     </c:if>         
                     </c:forEach>
               ...
               <li>
                  ...
                  <div class="inner">                     
                     ...
                     <form action="scheduleModi.fl?gNo=${gNo}&sNo=${list.sNo}&sDate=${list.sDate}&sCont=${list.sCont}" method="get">
                     <div class="writeDown">
                        ...
                        <input type="button" value="삭제" class="btn" onclick="window.location='/filo/travel/scheduleDel.fl?gNo=${gNo}&sNo=${list.sNo}'" />
                        </c:if>
                     </div>
                     </form>
                     </c:forEach>                     
                           ...
                           <c:if test="${memId == grpSpace.id}">
                              ...
                              <c:if test="${!empty grpSpace.genReview}">
                                 ...
                                 <c:if test="${grpSpace.shared == 0}">   <!-- 그룹스페이스 쉐어드가 0이면 공개여부 보여줌 -->
                                    <form action="/filo/travel/groupSpace.fl" name="openOrbOpen" method="get">
                                       <input type="hidden" name="gNo" value="${grpSpace.gNo}" />
   ...
   function changeStatus(gNo, status){
      ...
      $.ajax({
         url: "/filo/travel/changeStatus.fl",
         type: "POST",
         dataType: "json",
         contentType: "application/json",
         data: JSON.stringify(data),
