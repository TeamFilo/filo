	...
	<div id="transcroller-body" class="wrapAll client">
		...
		<div class="makingListWrap">
			...
			<div class="titWrap">
				...
				<div class="goMake">
					<img src="<c:url value="/resources/images/tm/makingico3.png" />" class="bag bag3" data-aos="fade-down" data-aos-delay="200" data-aos-duration="400" />		
				</div>
			</div>
			</c:if>
			<c:if test="${id!=null}">
			<div class="titWrap">
				...
				<div class="goMake">
					<img src="<c:url value="/resources/images/tm/makingico2.png" />" class="bag bag2" data-aos="fade-down" data-aos-delay="200" data-aos-duration="400" />		
				</div>
				</c:if>
				<c:if test="${rkInfo.rkNo>1}">
				<p class="tit2" data-aos="fade-right" data-aos-delay="200" data-aos-duration="400">직접 여행 만들고 리더가 되어볼까요?</p>
				<div class="goMake">
					<a href="/filo/travel/makingWrite.fl"></a>
					<p class="txt">Click to making!</p>
					<img src="<c:url value="/resources/images/tm/makingico1.png" />" class="bag" data-aos="fade-down" data-aos-delay="200" data-aos-duration="400" />			
					<img src="<c:url value="/resources/images/tm/makingico_sh.png" />" class="bagsh" data-aos="fade-down" data-aos-delay="200" data-aos-duration="400" />
				</div>
				</c:if>
			...
			<div class="myMake">
				...
				<ul class="list">
					...
					<li>
						<a href="makingCont.fl?gNo=${wArticle.gNo}"></a>
				...
				<ul class="list">
					...
					<li>
						<a href="groupSpace.fl?gNo=${jArticle.gNo}"></a>
			...
			<div class="searchMake">
				<div class="inner">
					<!-- 검색창 -->
					<form id="searchForm" name="searchForm" onsubmit="return validation()" method="post" action="makingList.fl">			
				...
				<div class="pageNumbers">
					...
					<c:if test="${startD!=null && endD!=null}">
						<c:if test="${startPage>pageBlock}">
							<a href="makingList.fl?pageNum=${startPage-pageBlock}&startD=${startD}&endD=${endD}"> &lt; </a>
						</c:if>				
					</c:if>				
					<c:if test="${startD==null || endD==null}">
						<c:if test="${startPage>pageBlock}">
							<a href="makingList.fl?pageNum=${startPage-pageBlock}"> &lt; </a>
						</c:if>
					</c:if>
					<!-- 페이지번호 리스트 -->				
					<c:if test="${startD!=null && endD!=null}">
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="makingList.fl?pageNum=${i}&startD=${startD}&endD=${endD}">&nbsp; ${i} &nbsp;</a>
						</c:forEach>
					</c:if>
					<c:if test="${startD==null || endD==null}">
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="makingList.fl?pageNum=${i}">&nbsp; ${i} &nbsp;</a>
						</c:forEach>
					</c:if>
					<!-- 뒤로 가는 기호 -->				
					<c:if test="${startD!=null && endD!=null}">
