...
public class MessageController {
	...
	public String message(Model model) {
		...
		return "tm/client/message/message";
	}
	
	
	// 메세지 작성
	@RequestMapping("messageWrite.fl")
	public String messageWrite(String receiver, Model model, String reply) {
		System.out.println("답장받는사람" + reply);
		
		model.addAttribute("receiver",receiver);
		model.addAttribute("reply", reply);
		return "tm/client/message/messageWrite";
	}
	
	
	// 메세지 작성 pro
	@RequestMapping("messageWritePro.fl")
	public String messageWritePro(MessageDTO msgDto, Model model) {
		...
		return "tm/client/message/messageWritePro";
	}
	
	
	//메세지 삭제
	@RequestMapping("messageDel.fl")
	...
	//	int mNo = Integer.parseInt(msgNo);
		...
		//result 1이면 삭제 완료
		model.addAttribute("result", result);
		return "tm/client/message/messageDel";
	}
